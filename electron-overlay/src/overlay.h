...
class OverlayMain : public IIpcHost
{
  ...
  public:
    ...
    void notifyGameEvent(std::uint32_t pid, const std::string& eventName, const std::string& eventData)
    {
        if (eventCallback_)
        {
            Napi::HandleScope scope(eventCallback_->env);
            Napi::Object object = Napi::Object::New(eventCallback_->env);
            object.Set("pid", Napi::Value::From(eventCallback_->env, pid));
            object.Set("eventName", Napi::Value::From(eventCallback_->env, eventName));
            object.Set("eventData", Napi::Value::From(eventCallback_->env, eventData));
            eventCallback_->callback.MakeCallback(eventCallback_->receiver.Value(), { Napi::Value::From(eventCallback_->env, "game.event"), Napi::Value::From(eventCallback_->env, object) });
        }
    }
...
break;
            ...
            switch (stdxx::hash(ipcMsg.type.c_str()))
            {
                ...
                OVERLAY_DISPATCH("game.event", GameEvent);
            default:
                break;
            }
        }
    ...
    void _onGameEvent(std::uint32_t pid, const std::shared_ptr<overlay::GameEvent>& overlayMsg)
    {
        node_async_call::async_call([this, pid, overlayMsg]() {
            notifyGameEvent(pid, overlayMsg->eventName, overlayMsg->eventData);
        });
    }
};
