...
static CAddress FindDataSSE(uint8_t* data, std::size_t size, const uint8_t* needle, std::size_t needle_size)
{
    ...
    FindDataSSEImpl(data, size, needle, needle_size,
                    [&result, base](CAddress match) {
                        result = match - base;
                        return scan_detail::SearchAction::Stop;
                    });
    return result;
}
...
static std::vector<CAddress> FindDataMultiSSE(uint8_t* data, std::size_t size, const uint8_t* needle, std::size_t needle_size)
{
    ...
    FindDataSSEImpl(data, size, needle, needle_size,
                    [&results, base](CAddress match) {
                        ...
                        return scan_detail::SearchAction::Continue;
                    });
    return results;
}
...
CAddress scan::FindPattern(uint8_t* data, std::size_t size, std::string_view pattern) noexcept
{
    ...
    if (s_InstructionSet.SupportAvx2())
        return scan_detail::FindPatternAVX2(data, size, pat);

    return scan_detail::FindPatternSSE(data, size, pat);
}
...
std::vector<CAddress> scan::FindPatternMulti(uint8_t* data, std::size_t size, std::string_view pattern) noexcept
{
    ...
    if (s_InstructionSet.SupportAvx2())
        return scan_detail::FindPatternMultiAVX2(data, size, pat);

    return scan_detail::FindPatternMultiSSE(data, size, pat);
}
...
CAddress scan::FindStr(uint8_t* data, std::size_t size, const std::string& str, bool zero_terminated, bool exact) noexcept
{
    ...
    if (s_InstructionSet.SupportAvx2())
    {
        scan_detail::FindDataAvx2Impl(data, size, needle, needle_size, callback);
    }
    else
    {
        scan_detail::FindDataSSEImpl(data, size, needle, needle_size, callback);
    }
...
CAddress scan::FindPtr(std::uintptr_t data, std::size_t size, std::uintptr_t ptr) noexcept
{
    ...
    if (s_InstructionSet.SupportAvx2())
    {
        scan_detail::FindValueAVX2Impl(data, size, ptr, callback);
    }
    else
    {
        scan_detail::FindValueSSEImpl(data, size, ptr, callback);
    }
...
CAddress scan::FindRVA(std::uintptr_t data, std::size_t size, uint32_t rva) noexcept
{
    ...
    if (s_InstructionSet.SupportAvx2())
    {
        scan_detail::FindValueAVX2Impl(data, size, rva, callback);
    }
    else
    {
        scan_detail::FindValueSSEImpl(data, size, rva, callback);
    }
...
{
    if (s_InstructionSet.SupportAvx2()) return scan_detail::FindDataAVX2(data, size, needle, needle_size);

    return scan_detail::FindDataSSE(data, size, needle, needle_size);
}

std::vector<CAddress> scan::FindDataMulti(uint8_t* data, std::size_t size, const uint8_t* needle, std::size_t needle_size) noexcept
{
    if (s_InstructionSet.SupportAvx2()) return scan_detail::FindDataMultiAVX2(data, size, needle, needle_size);

    return scan_detail::FindDataMultiSSE(data, size, needle, needle_size);
}
