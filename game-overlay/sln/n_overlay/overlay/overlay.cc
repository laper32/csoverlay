...
void OverlayConnector::sendGameEvent(const std::string& eventName, const std::string& eventData)
{
    HookApp::instance()->async([this, eventName, eventData]() {
        _sendGameEvent(eventName, eventData);
    });
}

bool OverlayConnector::isConnected() const
{
    return ipcLink_ != nullptr && ipcLink_->isConnect();
}
...
void OverlayConnector::_sendGameEvent(const std::string& eventName, const std::string& eventData)
{
    overlay::GameEvent message;
