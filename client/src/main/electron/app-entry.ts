...
class Application {
  ...
  public startOverlay() {
    ...
    this.Overlay!.setEventCallback((event: string, payload: any) => {
      if (event === "game.process") {
        console.log(`[DLL Injected] PID=${payload.pid} path=${payload.path}`);
        if (this.mainWindow) {
          this.mainWindow.webContents.send("status", `DLL injected into: ${payload.path} (PID ${payload.pid})`);
        }
        ...
        console.log(`[GameEvent] ${payload.eventName}: ${payload.eventData}`);
        // Forward to renderer for MVP animation
        if (this.mainWindow) {
          this.mainWindow.webContents.send("gameEvent", {
  ...
  public startCS2() {
    // Auto-start overlay if not already running
    if (!this.Overlay) {
      this.Overlay = loadNativeLib();
    }
    this.scaleFactor = screen.getDisplayNearestPoint({ x: 0, y: 0 }).scaleFactor;
    this.startOverlay();
    this.createOsrWindow();
    this.createOsrStatusbarWindow();

    const childProcess = require("child_process") as typeof import("child_process");
    const cs2Path = "C:\\Program Files (x86)\\Steam\\steamapps\\common\\Counter-Strike Global Offensive\\game\\bin\\win64\\cs2.exe";
    const args = ["-insecure", "-worldwide"];

    console.log(`Starting CS2: ${cs2Path} ${args.join(" ")}`);

    const proc = childProcess.spawn(cs2Path, args, {
      detached: true,
      stdio: "ignore",
    });
    proc.unref();

    // Wait for CS2 window to appear, then inject
    const injectInterval = setInterval(() => {
      const windows = this.Overlay.getTopWindows();
      for (const w of windows) {
        if (w.title && w.title.toLowerCase().includes("counter-strike")) {
          console.log(`Injecting into CS2: ${JSON.stringify(w)}`);
          this.Overlay.injectProcess(w);
    ...
    setTimeout(() => clearInterval(injectInterval), 60000);
  }
